	...
	def train_model(self):
		...
		for epoch in range(1, params.n_epochs+1):
			...
			if epoch % 5 == 0:
				...
				self.test_model(batch, epoch)

	def test_model(self, batch, epoch):
		tloss = 0
		t_batches = 0
		yt = []
		yp = []
		self.model.cuda(self.model.network).eval()
		start_time = dt.now()
